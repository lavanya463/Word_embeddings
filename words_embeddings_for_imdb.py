...
def save_the_model(model, filename):
...
def plot_the_embeds(model):
    ...
    keys = ['bollywood', 'good', 'bad', 'actor', 'julia', 'script', 'story', 'baby', 'season', 'president']
    print(keys)
...
def main():
    ...
    for embeddings_dim in [64]:
        for window in [5]:
            for min_count in [50]:
                combination = "embedding_dim"+str(embeddings_dim)+"_window"+str(window)+"_min_count"+str(min_count)
                model = train_word2vec(data_sentences, embeddings_dim, window, workers, min_count)
                filename = save_the_model(model, "embeddings_word2vec.txt")
                ...
                accuracy = create_train_nn(x_train, y_train, x_test, y_test, num_words, embeddings_dim, embedding_matrix, max_length)
                panda_data.append([combination, accuracy])
    df = pd.DataFrame(panda_data, columns=['combination', 'accuracy'])
    df.to_excel("combination_results.xlsx")
    """
    # plotting
